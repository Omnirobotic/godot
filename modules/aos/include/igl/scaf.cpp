                   ...
                   const Eigen::MatrixXd &uv_init)
{
  ...
  for (auto cur_bnd : all_bnds)
  {
    ...
    s.internal_bnd.bottomRows(cur_bnd.size()) = Eigen::Map<ArrayXi>(cur_bnd.data(), cur_bnd.size()) + s.mv_num;
...
void solve_weighted_arap(SCAFData &s, Eigen::MatrixXd &uv)
{
  ...
  uv = Eigen::Map<Matrix<double, -1, -1, Eigen::ColMajor>>(Uc.data(), v_n, dim);
}
