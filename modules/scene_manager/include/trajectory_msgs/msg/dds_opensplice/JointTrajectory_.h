//******************************************************************
// 
//  Generated by IDL to C++ Translator
//  
//  File name: JointTrajectory_.h
//  Source: trajectory_msgs\msg\dds_opensplice\JointTrajectory_.idl
//  Generated: timestamp removed to make the build reproducible
//  OpenSplice 6.9.190403OSS
//  
//******************************************************************
#ifndef _JOINTTRAJECTORY__H_
#define _JOINTTRAJECTORY__H_

#include "sacpp_mapping.h"
#include "JointTrajectoryPoint_.h"
#include "Header_.h"
#include "trajectory_msgs/msg/rosidl_typesupport_opensplice_cpp__visibility_control.h"


namespace trajectory_msgs
{
   namespace msg
   {
      namespace dds_
      {
         struct JointTrajectory_;

         struct ROSIDL_TYPESUPPORT_OPENSPLICE_CPP_PUBLIC_trajectory_msgs JointTrajectory_
         {
               struct _joint_names__seq_uniq_ {};
               typedef DDS_DCPSUStrSeqT <struct _joint_names__seq_uniq_> _joint_names__seq;
               typedef DDS_DCPSUStrSeq_var < _joint_names__seq> _joint_names__seq_var;
               typedef DDS_DCPSUStrSeq_out < _joint_names__seq> _joint_names__seq_out;
               struct _points__seq_uniq_ {};
               typedef DDS_DCPSUVLSeq < JointTrajectoryPoint_, struct _points__seq_uniq_> _points__seq;
               typedef DDS_DCPSSequence_var < _points__seq> _points__seq_var;
               typedef DDS_DCPSSequence_out < _points__seq> _points__seq_out;
               ::std_msgs::msg::dds_::Header_ header_;
               _joint_names__seq joint_names_;
               _points__seq points_;
         };

         typedef DDS_DCPSStruct_var < JointTrajectory_> JointTrajectory__var;
         typedef DDS_DCPSStruct_out < JointTrajectory_> JointTrajectory__out;
      }
   }
}




#endif
